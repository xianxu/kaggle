...
### 2026-07-02 — M2 boundary review (FIX-THEN-SHIP → fixed)
- **Verdict FIX-THEN-SHIP, zero Critical.** The one Important finding + cheap coverage/quality items folded in before crossing:
  - **Terminal-error fast-fail (Important).** The Task-3 "timeout contract" modeled only pending→scored→exhaustion; a submission Kaggle *rejects* gets `Status="error"` permanently, and `LatestScored` (nil score) would poll the entire budget (30×5s) then misattribute the message. `pollScore` now returns immediately on a newest-row `StatusError` (`scored=false`), and `run()` emits a distinct "submission rejected (status=error)" error vs the timeout message. Both still write `submission.json` + exit non-zero. On the deferred live path (the fake never emits `error`), but cheap and correct before kbench#1's first live run. Unit-tested (`TestPollScore_TerminalErrorFastFails`: 1 poll, no sleep).
  - **Coverage (review §5):** added the zip-slip adversarial unit test (`TestUnzip_RejectsZipSlip`) and submit's missing-upstream-artifact error path (`TestRun_MissingUpstreamArtifact`).
  - **Minor:** `envInt`/`envDuration` now warn to stderr on a malformed value (was a silent default — a hidden misconfig); `stepio_test` uses `strings.Contains` instead of a hand-rolled helper.

### 2026-07-02 — whole-issue close review (FIX-THEN-SHIP → Critical fixed)
- **Verdict FIX-THEN-SHIP; one Critical (fixed) the milestone reviews missed.** `kaggle/submit` selected its score via `kaggle.LatestScored` (**any** scored row, newest-first) — uncorrelated to the file it just uploaded. On a real competition with prior scored submissions, poll #1 would report an **older** submission's score for our still-pending upload (silent wrong output; `parse_test.go` literally encodes the selection). Structurally unreachable by the single-submission fake, so the fake-verified close is honest — but it defeats submit's purpose (`ARCH-PURPOSE`: "return a score" vs "return the score of what we submitted"), so fixed **now** rather than deferring to kbench#1's live run.
  - **Fix:** `pollScore` now keys off the **newest** row (`subs[0]` — the just-uploaded one, Kaggle lists newest-first), filename-correlated to the uploaded file; it waits for OUR row to score and never reports another submission's score (if ours isn't visible yet / a concurrent submit raced in, it keeps polling → safe timeout). `kaggle.LatestScored` stays as a "best current score across all submissions" library query with a doc-note that it is **not** the submit-correlation selector.
  - **Testability (the Important):** the single-submission fake couldn't reach the bug, so added `KAGGLE_FAKE_PRIOR_SCORE` (the fake emits a pre-existing older scored row) + two regressions — a pure `TestPollScore_ReportsOwnSubmissionNotPriorScored` and an end-to-end `TestRun_ReportsOwnScoreWithPriorScoredSubmission` (both fail under the old `LatestScored` logic).
  - **Minor:** `gofmt -w` (a comment-alignment violation `gofmt -l` flagged). `go test ./...` + `go vet` + `gofmt -l` all clean.
